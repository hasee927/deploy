...
async def idbi(data ,db):
    ...
    for index, row in csv.iterrows():
        tmpData = str(row['Unnamed: 5'])
        if tmpData.find('UPI') > -1:
            # 去掉金额中的逗号
            tmp_amount = str(row['Unnamed: 9']).strip(" ")
            amount = str(float(''.join(tmp_amount.split(','))))
            tmp_balance = str(row['Unnamed: 10']).strip(" ")
            ...
            post_data = {
                ...
                "bank_utr": utr,
                "trading_info": str(row['Unnamed: 5'].strip(" ")), #流水详情
                "amount": amount,
                "channel_code": str(data.channel_code),
                "trading_time": str(row['Unnamed: 3'].strip(' ')),
