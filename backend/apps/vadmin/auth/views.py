...
@app.get("/user/current/google/qr", summary="生成google验证二维码")
async def user_current_google_qr(auth: Auth = Depends(AllUserAuth())):
    return SuccessResponse(await crud.UserDal(auth.db).create_google_qr(auth.user))
