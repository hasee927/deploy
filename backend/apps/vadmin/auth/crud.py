...
class UserDal(DalBase):
    ...
    # 生成google验证二维码
    async def create_google_qr(self, user: models.VadminUser) -> str:

        # if not user.secret and user.is_reset_password == 0:
        if user.is_reset_password == 0:
            ...
            data = f"otpauth://totp/YG:{name}?secret={secret}&issuer=YG"

            url = pyqrcode.create(data)
            stream = BytesIO()
            url.png(stream, scale=3)
            qr_image = f"data:image/png;base64,{ base64.b64encode(stream.getvalue()).decode('utf-8') }"
            return qr_image
